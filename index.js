...
const multerStorage = multer.diskStorage({
    destination: function (req, file, cb) {
        if (file.fieldname === 'other') {
            cb(null, 'media/other/');
        } else if (file.fieldname === 'creation') {
...
app.post('/creation/add', isAuthenticated, multerParser, upload.fields([{ name: 'creation', maxCount: 9 }]), (req, res) => {
...
app.get('/creation/topic/:id', (req, res) => {
    ...
    pool.query(`SELECT forum_creation.*, users.username, users.skin FROM forum_creation JOIN users ON forum_creation.owner = users.id WHERE identifier = $1 LIMIT 1;`, [req.params.id], async (err, result) => {
        ...
        const pictures = await pool.query('SELECT picture FROM forum_creation_pictures WHERE topic_id = $1;', [result.rows[0].id]);

        res.render('topic-creation', { user: req.user, topics: result.rows, footer: footer_html, pictures: pictures.rows });
    });
});
